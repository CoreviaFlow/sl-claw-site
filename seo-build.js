...
const UI = {
  ru:{ catalog:'Каталог ниш', pricing:'Тарифы', how:'Как это работает', cabinet:'Кабинет ↗',
       ...
       closerlead:'Это не автоответчик «приняли заявку», а продавец, который доводит до оплаты и возвращается сам:',
       closer:['Выставляет счёт прямо в переписке — данные берёт из вашей CRM или 1С','Скидывает драфт договора и коммерческое предложение под запрос клиента','Дожимает сделку: снимает последние возражения и подводит к оплате','Возвращается через время — проверяет статус сделки, переспрашивает по КП («рассмотрели предложение?») и аккуратно тревожит «дозревающего» клиента, пока тот не купит или не откажет'],
  ...
  uk:{ catalog:'Каталог ніш', pricing:'Тарифи', how:'Як це працює', cabinet:'Кабінет ↗',
       ...
       closerlead:'Це не автовідповідач «прийняли заявку», а продавець, який доводить до оплати й повертається сам:',
       closer:['Виставляє рахунок прямо в листуванні — дані бере з вашої CRM або 1С','Скидає драфт договору та комерційну пропозицію під запит клієнта','Дотискає угоду: знімає останні заперечення й підводить до оплати','Повертається згодом — перевіряє статус угоди, перепитує по КП («розглянули пропозицію?») і акуратно турбує «дозріваючого» клієнта, поки той не купить або не відмовиться'],
