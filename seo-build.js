...
const UI = {
  ru:{ catalog:'Каталог ниш', pricing:'Тарифы', how:'Как это работает', cabinet:'Кабинет ↗',
       ...
       feed:'В комплекте — навыки продаж и экспертиза этой ниши. Свои товары, цены и данные о компании вы добавляете сами и дальше обогащаете.',
  ...
  uk:{ catalog:'Каталог ніш', pricing:'Тарифи', how:'Як це працює', cabinet:'Кабінет ↗',
       ...
       feed:'У комплекті — навички продажів та експертиза цієї ніші. Свої товари, ціни та дані про компанію ви додаєте самі і далі збагачуєте.',
...
function page(n, v){
  ...
  const desc = `${ensurePeriod(f.tagline)} Готовый AI-продавец для ниши «${f.name}» ${v.geoWord}: ведёт диалог, снимает возражения, доводит до сделки. Разверни за час, плати за работу ИИ по факту.`;
  const descUk = `${ensurePeriod(f.tagline)} Готовий AI-продавець для ніші «${f.name}» ${v.geoWord}: веде діалог, знімає заперечення, доводить до угоди. Розгорни за годину.`;
...
<meta name="keywords" content="${esc(v.lang==='uk'?`AI-продавець для ${f.name}, чат-бот для ${f.name}, автоматизація продажів ${f.name}, ${f.name} бот ${v.geoWord}`:`AI-продавец для ${f.name}, чат-бот для ${f.name}, автоматизация продаж ${f.name}, ${f.name} бот ${v.geoWord}`)}">
